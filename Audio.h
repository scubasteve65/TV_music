...
//#define NOISEFLOOR 50 // TODO THIS IS LOW but microphone seems not pick up much
//#define NOISEFLOOR 65
// SLH change from 65 on 10/20
...
void initializeAudio() {
  ...
  Serial.println ("in initializeAudio");
...
//      Serial.print("spectrumValue[i] < NOISEFLOOR: ");Serial.println(spectrumValue[i]);
      spectrumValue[i] = 0;
    } else {
...
//  Serial.print("specCombo: ");Serial.println(specCombo);
  
//  Serial.print("beatDetect: specCombo - beatAvg > beatLevel ");
//  Serial.print(specCombo); Serial.print(" - ");Serial.print(beatAvg);Serial.print(" = ");Serial.print(specCombo - beatAvg);Serial.print(" > ");Serial.println(beatLevel);

//  if((specCombo - beatAvg) > beatLevel){
//    // should detect this beat but maybe we just previously did
////    Serial.print("beatDetect: specCombo - beatAvg > beatLevel:  ");
////    Serial.print(specCombo - beatAvg);Serial.print(" > ");Serial.println(beatLevel);
//////    Serial.print("beatTriggered = ");Serial.println(beatTriggered);
////    Serial.print("currentMillis - lastBeatMillis >= beatDelay:  ");Serial.print(currentMillis - lastBeatMillis);Serial.print(" > ");Serial.println(beatDelay);
//  }
  ...
  if ((specCombo - beatAvg) > beatLevel && 
       beatTriggered == 0 && 
       currentMillis - lastBeatMillis > beatDelay) {
    beatTriggered = 1;
...
byte trebDetect() {
  ...
  //float specCombo = spectrumDecay[trebBand] / 1.0f;
  trebAvg = (1.0 - AGCSMOOTH) * trebAvg + AGCSMOOTH * specCombo;

  if (lastTrebVal < trebAvg) 
    lastTrebVal = trebAvg;
  if ((specCombo - trebAvg) > trebLevel && 
      trebTriggered == 0 && 
      currentMillis - lastTrebMillis > trebDelay) {
    trebTriggered = 1;
