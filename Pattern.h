...
void saveBands(uint16_t in){
  //Serial.print("saving bands: ");
  for(byte b=0; b<7; ++b) {
  //Serial.print(" spectrumByte[");Serial.print(b);Serial.print("]: ");Serial.print(spectrumByte[b]);
  ...
  //Serial.print(" smp[6][idx]: ");Serial.print(smp[6][idx]);
  //Serial.print(" = spectrumByte[6]: ");Serial.print(spectrumByte[6]);
...
void analyzeBands(){
  ...
  if(bt && (newTreb!=trebBand)) 
    bt = false;
  else if(!bt && newTreb != trebBand){
    Serial.print("New Treble band: ");Serial.println(newTreb);
    trebBand = newTreb;
    bt = true;
  }
  
  // 2 Strikes Against The Bass Band
  if( bb && (newBass != bassBand)) {
    bb = false;
  } 
  else if(!bb && newBass!=bassBand){
    Serial.print("New Bass band: ");Serial.println(newBass);
    bassBand = newBass;
    bb = true;
  }
}
...
//k = 0;
  ...
  if(beatDetect()){
    ...
    if(trebDetect()){
      Serial.println("trebble detected ");
      rgbRate+=8; 
    }

    huey += rgbRate;
    leds[ok] = ColorFromPalette(RainbowColors_p, huey, brightness, LINEARBLEND);
  } 
  if(brightness == 0) {
